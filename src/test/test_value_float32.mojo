...
fn test_iadd() raises:
    ...
    var tb: PythonObject = torch.Tensor([-3]).double()
    ta += tb
    var tc: PythonObject = ta
    tc.backward()
    assert_equal(
        c.data.get_data_copy(),
        GradFloat32(atof(tc.data.item())),
        msg="Forward pass disagrees with Pytorch",
    )
    assert_equal(
        a.grad.get_data_copy(),
...
fn test_more_ops() raises:
    ...
    var c = a + b
    var d = a * b + b**3
    c += c + Value(GradFloat32(1))
    c += Value(GradFloat32(1)) + c + (-a)
    d += d * Value(GradFloat32(2)) + (b + a).relu()
    d += Value(GradFloat32(3)) * d + (b - a).relu()
    var e = c - d
    var f = e**2
    var g = f / Value(GradFloat32(2.0))
    g += Value(GradFloat32(10.0)) / f
    g.backward()
